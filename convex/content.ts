...
export const getFeaturedContent = query({
  ...
  handler: async (ctx) => {
    ...
    // Sort to put volleyball first, then the rest by creation time
    const sorted = featured.sort((a, b) => {
      // Put volleyball (McLennan Community College Volleyball) first
      if (a.title === "McLennan Community College Volleyball") return -1;
      if (b.title === "McLennan Community College Volleyball") return 1;
      // For all others, maintain original order (by creation time)
...
export const updateContent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Update the content with provided fields
